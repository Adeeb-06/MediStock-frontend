...
const SellStock = () => {
  ...
  const handleRemove = (removeIndex) => {
    setStocksInput(stocksInput.filter((_, i) => i !== removeIndex));
    unregister(`stocks.${removeIndex}`);
  };
  const onsubmit = async (data) => {
    // setIsLoading(true);
    console.log(data.stocks, '1')
    // console.log(stocksInput, '2')
                    ...
                    <div className="relative flex gap-4">
                      ...
                      <select
                        {...register(`stocks.${index}.medicine`, {
                          required: { value: true, message: "Select a medicine" }
                        })}
                        // defaultValue={stock.medicine}
